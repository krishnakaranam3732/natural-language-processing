...
def getTypedDependencies(totalData):
    ...
    for each in final_sentences:
        typed = typedDependencies.raw_parse_sents([each])
        result += [typed]
    return result
...
for eachFile in os.listdir(directory):
    if eachFile.endswith(".txt"):
        ...
        for each in fileData:
            dependencies += getTypedDependencies(each)
        for each in dependencies:
            rootcount += countRoot(each)
        print("the sentence count is: ",rootcount)
